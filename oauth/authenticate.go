...
func (s *Service) Authenticate(token string) (*models.OauthAccessToken, error) {
	...
	notFound := s.db.Where("token = ?", token).First(accessToken) == nil
...
func (s *Service) ClearUserTokens(userSession *session.UserSession) {
	...
	found := models.OauthRefreshTokenPreload(s.db).Where("token = ?", userSession.RefreshToken).First(refreshToken) != nil
	if found {
	...
	accessToken := new(models.OauthAccessToken)
	found = models.OauthAccessTokenPreload(s.db).Where("token = ?", userSession.AccessToken).First(accessToken) != nil
	if found {
